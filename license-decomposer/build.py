...
APP = ['src/main.py']
DATA_FILES = [
    ('', ['icons/jfrog_icon.icns']),
]
OPTIONS = {
    'argv_emulation': False,
    'iconfile': 'icons/jfrog_icon.icns',
    'plist': {
...
def build_app():
    ...
    # 复制必要的文件到临时目录
    shutil.copy2("icons/jfrog_icon.icns", temp_build_dir)
    
    # 复制src目录到临时目录
    if os.path.exists("src"):
        shutil.copytree("src", os.path.join(temp_build_dir, "src"))
        print("✅ 复制src目录到临时构建目录")
    
    # 复制icons目录到临时目录
    if os.path.exists("icons"):
        shutil.copytree("icons", os.path.join(temp_build_dir, "icons"))
        print("✅ 复制icons目录到临时构建目录")
...
'''
    ...
    # 在临时构建目录中创建一个 __init__.py 文件，使 src 成为一个包
    init_file = os.path.join(temp_build_dir, "src", "__init__.py")
    if not os.path.exists(init_file):
        with open(init_file, "w") as f:
            f.write("# This file makes src a Python package\n")
        print("✅ 创建 src/__init__.py 文件")
    ...
    try:
        ...
        # 确保图标文件在正确位置
        icon_source = "icons/jfrog_icon.icns"
        ...
        # 复制src目录中的所有Python文件到应用程序包
        src_source_dir = "src"
        src_target_dir = os.path.join(target_app, "Contents", "Resources", "src")
        
        if os.path.exists(src_source_dir):
            # 确保目标目录存在
            os.makedirs(src_target_dir, exist_ok=True)
            
            # 复制所有Python文件
            for py_file in os.listdir(src_source_dir):
                if py_file.endswith('.py'):
                    source_py = os.path.join(src_source_dir, py_file)
                    target_py = os.path.join(src_target_dir, py_file)
                    shutil.copy2(source_py, target_py)
                    print(f"✅ 复制Python文件: {py_file}")
